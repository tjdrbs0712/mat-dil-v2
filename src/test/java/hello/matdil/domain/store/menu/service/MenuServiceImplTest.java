...
class MenuServiceImplTest {
    ...
    void 가게가_존재하지_않으면_예외를_던진다() {
        // given
        given(storeReader.readById(1L, 1L, UserRole.USER)).willThrow(new StoreException(StoreErrorCode.STORE_NOT_FOUND));
    ...
    void 메뉴_리스트를_정상적으로_조회하고_조립한다() {
        // given
        given(storeReader.readById(1L, 1L, UserRole.OWNER)).willReturn(store);
